...
pub struct PartialMap {
    ...
    // 確定していないconnectionを許す
    pub connections: Vec<Option<crate::finalize_map::GuessConnection>>,
}
...
/// 入口: 判定→構成
pub fn finalize_guess_map(merge: &MergeResult) -> PartialReport {
    // A: ID 完了チェック
    ...
    let mut connections: Vec<Option<crate::finalize_map::GuessConnection>> = Vec::new();
    ...
    for u in 0..n {
        for d in 0..6u8 {
            if let Some(v) = delta[u][d as usize] {
                ...
                if push_this {
                    connections.push(Some(crate::finalize_map::GuessConnection {
    ...
    PartialReport {
        map: PartialMap { rooms, starting_room, connections },
        stats: FinalizeStats {
