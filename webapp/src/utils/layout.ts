 ...
 */
export function calculateRoomLayout(map: Map, containerWidth: number, containerHeight: number): RoomLayout[] {
 ...
 */
function circularLayout(map: Map, containerWidth: number, containerHeight: number): RoomLayout[] {
  const roomCount = map.rooms.length;
 ...
 */
function forceDirectedLayout(map: Map, containerWidth: number, containerHeight: number): RoomLayout[] {
  const roomCount = map.rooms.length;
  const layout: RoomLayout[] = [];
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    for (let i = 0; i < roomCount; i++) {
      ...
      // Keep within bounds
      const margin = 100;
      layout[i].position.x = Math.max(margin, Math.min(containerWidth - margin, layout[i].position.x));
      layout[i].position.y = Math.max(margin, Math.min(containerHeight - margin, layout[i].position.y));
    }
  }
 ...
 */
export function getLayoutBounds(layout: RoomLayout[]): { minX: number, maxX: number, minY: number, maxY: number } {
  if (layout.length === 0) {
