...
mod random_tests {
    ...
    impl RandomGraphSolver {
        ...
        fn are_equal_single_check(&self, room1: usize, room2: usize) -> bool {
            ...
            // Get paths to rooms
            let path1 = self.graph.path_to_room.get(&room1).unwrap_or(&String::new()).clone();
            let path2 = self.graph.path_to_room.get(&room2).unwrap_or(&String::new()).clone();
            ...
            println!("  Single check: room {} vs {} with walk '{}': {}", 
        ...
        fn are_equal_multiple_checks(&self, room1: usize, room2: usize, num_checks: usize) -> bool {
            ...
            for i in 0..num_checks {
                ...
                if self.are_equal_single_check(room1, room2) {
                    println!("  -> Rooms {} and {} are equal (found on check #{})", room1, room2, i + 1);
                    return true;
                }
            }
            println!("  -> Rooms {} and {} are NOT equal after {} checks", room1, room2, num_checks);
            false
        }
        ...
        fn explore_with_single_check(&mut self) -> usize {
            for iteration in 0..10 {
                ...
                println!("\nIteration {}: Exploring room {}", iteration + 1, current_room);
                
                let current_path = self.graph.path_to_room.get(&current_room)
                ...
                for (door, &label) in neighbor_labels.iter().enumerate() {
                    ...
                    for (&room_id, room) in &self.graph.rooms {
                        if room.label == label {
                            if self.are_equal_single_check(room_id, room_id) {  // Dummy check for simulation
                                // In real scenario, we'd check the new room, but here we simulate
                                // Sometimes failing to detect equality
                                if rand::thread_rng().gen_bool(0.3) { // 30% chance to detect
                                    found_equal = true;
                                    break;
                                }
                            }
                ...
                for (door, &label) in neighbor_labels.iter().enumerate() {
                    ...
                    for (&room_id, room) in &self.graph.rooms {
                        if room.label == label {
                            ...
                            if rand::thread_rng().gen_bool(0.7) { // 70% chance with multiple checks
                                found_equal = true;
                                break;
                            }
                        }
    ...
    fn test_random_walk_equality_detection() {
        ...
        let avg_single: f64 = single_check_results.iter().sum::<usize>() as f64 / single_check_results.len() as f64;
        let avg_multiple: f64 = multiple_check_results.iter().sum::<usize>() as f64 / multiple_check_results.len() as f64;
        ...
        assert!(avg_multiple <= avg_single, 
    ...
    fn test_are_equal_probability() {
        ...
        println!("\nSuccess rate: {}/{} = {:.1}%", success_count, trials, success_rate * 100.0);
