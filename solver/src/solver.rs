...
impl Solver {
    ...
    pub async fn are_equal(&self, path1: &str, path2: &str) -> Result<bool> {
        ...
        for attempt in 0..self.max_tries {
            ...
            println!("        Plan 1 starts with: '{}'", &plan1[..plan1.len().min(10)]);
            println!("        Plan 2 starts with: '{}'", &plan2[..plan2.len().min(10)]);
            ...
            println!("        Walk from path 1: {:?}{}",
                     &walk1[..display_len],
                     if walk1.len() > display_len { "..." } else { "" });
            println!("        Walk from path 2: {:?}{}",
                     &walk2[..display_len],
    ...
    pub async fn explore(&mut self, problem_size: usize) -> Result<()> {
        ...
        self.frontier.push_back((self.graph.starting_room, String::new()));
            ...
            if let Some((current_room, current_path)) = self.frontier.pop_front() {
                ...
                for (door_num, &label) in neighbor_labels.iter().enumerate() {
                    ...
                    for &existing_room in &existing_rooms {
                        ...
                        let existing_path = self.graph.path_to_room.get(&existing_room).cloned().unwrap_or_default();
                        
                        if self.are_equal(&neighbor_path, &existing_path).await.unwrap_or(false) {
