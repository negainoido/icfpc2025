...
mod tests {
    ...
    impl MockSolver {
        ...
        async fn are_equal(&mut self, room1: usize, room2: usize) -> Result<bool, String> {
            ...
            // First check labels
            let label1 = self.graph.rooms.get(&room1).ok_or("Room1 not found")?.label;
            let label2 = self.graph.rooms.get(&room2).ok_or("Room2 not found")?.label;
        ...
        async fn explore(&mut self, max_iterations: usize) -> Result<(), String> {
            for iteration in 0..max_iterations {
                ...
                println!(
                    "Exploring room {} (label {})",
                    current_room, self.graph.rooms[&current_room].label
                );
                ...
                for (door, label_opt) in neighbor_labels.iter().enumerate() {
                    if let Some(label) = label_opt {
                        ...
                        let existing_rooms: Vec<(usize, u8)> = self
                            .graph
                            .rooms
                            .iter()
                        ...
                        if !found_existing {
                            ...
                            let path =
                                format!("{}{}", self.graph.path_to_room[&current_room], door);
                ...
                for (door, target) in door_targets.iter().enumerate() {
                    if let Some(target_room) = target {
                        self.graph
                            .connect_rooms(current_room, door, *target_room, 0);
                    }
                }
    ...
    async fn test_probatio_should_have_3_rooms() {
        ...
        assert_eq!(
            room_count, 3,
            "Probatio should have exactly 3 rooms, but got {}",
            room_count
        );
    ...
    async fn test_equivalence_checking() {
        ...
        assert_ne!(
            walk1, walk4,
            "Paths to different rooms should have different random walks"
        );
    }
    ...
    async fn test_newly_created_rooms_should_be_checked_against_each_other() {
        ...
        for (door, &label) in neighbors.iter().enumerate() {
            ...
            for &(existing_id, existing_label) in &new_rooms {
                if existing_label == label {
                    ...
                    println!(
                        "Door {} should connect to previously created room {}",
                        door, existing_id
                    );
                    break;
                }
        ...
        assert_eq!(
            new_rooms.len(),
            2,
            "Should create only 2 new rooms for 2 unique labels"
        );
    }
}
